# appointment-domain Specification
...
The system SHALL support storing appointment times with timezone information.

#### Scenario: Appointment time is stored with timezone
- **GIVEN** a local date/time and timezone for an appointment
- **WHEN** creating the appointment
- **THEN** the time is stored with the provided timezone
- **AND** timezone information is preserved

**Note**: The domain layer accepts timezone as a parameter. The application layer is responsible for providing the configured timezone (Europe/Berlin) from configuration.
...
#### Scenario: Only scheduled appointments can be cancelled
- **GIVEN** an appointment with CANCELLED or COMPLETED status
- **WHEN** attempting to cancel
- **THEN** the operation fails

**Note**: Appointment completion is determined automatically when the appointment time passes, not through an explicit operation. The COMPLETED status is derived from time-based logic in the application layer.
...
**Note**: AppointmentCompleted events are defined in the domain but generated by the application layer when appointment completion is detected (when time passes). Domain operations return events using the Effect pattern: `Result<Effect<Appointment, DomainEvent>>`.
